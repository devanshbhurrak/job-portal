        ...
        <table className='w-full max-w-4xl bg-white border border-gray-200 max-sm:text-sm'>
          <thead>
            <tr className='border-b'>
              ...
              <th className='py-2 px-4 text-left'>Aciton</th>
            </tr>
          </thead>
          ...
          <tbody>
            {applicants.filter(item => item.jobId && item.userId).map((applicant, index) => (
              <tr key={index} className='text-gray-700 border-b'>
                ...
                <td className='py-2 px-4  text-center items-center flex'>
                  <img className='w-10 h-10 rounded-full mr-3 max-sm:hidden' 
                  src={applicant.userId.image} alt="" />
                  <span>{applicant.userId.name}</span>
                </td> 
                <td className='py-2 px-4 text-center max-sm:hidden'> {applicant.jobId.title} </td>
                <td className='py-2 px-4 text-center max-sm:hidden'> {applicant.jobId.location} </td>
